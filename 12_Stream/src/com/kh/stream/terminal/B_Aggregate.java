...
/*
 * 기본 집계
 *  - 최종 처리 기능으로 요소들을 개수, 합계, 
 *  평균값, 최대값, 최소값 등과 같이 하나의 값으로 산출하는 역할
 *  
 *  - count() : 요소의 개수를 리턴
 *  - sum () : 요소들의 합계를 리틴
 *  - average() : 요소들의 평균을 리턴
 ...
 *  
 *  커스텀 집계
 *  - 스트림에서 기본 집계 메서드를 제공하지만
 *    다양한 집계 결과물을 만들 수 있드록 reduce() 메서드 제공
 * */
public class B_Aggregate {
	...
	public static void main(String[] args) {
		...
		/*
		 * Optional 클래스 - 스트림의 최종 결과 값을 저장하는 객체 - 단순히 값만 저장하는 것이 아니라,
		 *  값의 존재 여부를 확인하고 값이 존재하지 않을 경우 디폴트 값을 설정할 수 있는 객체
		 *  
		...
		System.out.println("평균 : " + average.getAsDouble());// get해야 앞에 Optional 안나옴

		OptionalInt max = Arrays.stream(array).max();
		if (max.isPresent()) {// if문 사용
		...
		/*
		 * Arrays.stream(array).filter(value -> value % 2==0) .reduce((x,y)-> x *
		 * y).ifPresent(value -> System.out.println(value));
		 *  reduce는 계속 연산해주는 역할
		 * 위아래 같음
		 */

		IntStream stream = Arrays.stream(array);// 이건 변수 지정해서
