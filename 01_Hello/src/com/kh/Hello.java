 ...
 * - 모든 클래스는 하나의 패키지에 속하며, 패키지가 선언되지 않는 클래스는 
 *	 자동적으로 이름 없는 패키지(default)에 속하게된다.
 * 
 * */
package com.kh;
 ...
 * - 클래들이 모여 하나의 java 프로그름앨 구성한다.
 * */
public class Hello { // 클래스 명은 클래스 파일명과 동일해야한다.
	/*
	 * 메소드(method)
	 * - 클래스의 기능(동작)에 해당하는 구현 부분이다.
	 * - 해당 메소드 호출 시 메소드의 블럭({})에 해당하는 부분을 실행한다.
	 * */
	// 실행 메소드(main 메소드) : 자바 프로그램 실행시 java.exe에 의해 가장 먼저 호출한다.
 ...
 * - 클래스명 : 대문자로 시작.
 * 			  여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기한다.
 * - 메소드명 : 소문자로 시작.
 * 			   여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기
 * 
 *  * 자바의 특징
